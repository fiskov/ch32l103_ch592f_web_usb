...
/* MSC-era symbols still referenced by untouched parts of this EVT stack;
 * the demo never triggers them (no class requests are implemented). */
uint8_t UDisk_Out_Buf[ 512 ];
volatile uint8_t Udisk_Sense_Key, Udisk_Sense_ASC, Udisk_CSW_Status, Udisk_Transfer_Status;
#define DEF_UDISK_CSW_UP_FLAG 0x00
                    ...
                    if( R8_U2EP0_RX_CTRL & USBHS_UEP_R_SETUP_IS )
                    {
                        ...
                        if ( ( USBHS_SetupReqType & USB_REQ_TYP_MASK ) != USB_REQ_TYP_STANDARD )
                        {
                            ...
                            if (( USBHS_SetupReqType & USB_REQ_TYP_MASK ) == USB_REQ_TYP_VENDOR)
                            {
                               ...
                               uint16_t respLen = 0;
                               uint8_t vres = WinUSB_ProcessVendorRequest(
                                   USBHS_SetupReqCode, USBHS_SetupReqValue,
                                   USBHS_SetupReqIndex, USBHS_SetupReqLen,
                                   &pResp, &respLen );
          ...
          /* data-in stage processing */
            switch ( endp_num )
            {
                ...
                case DEF_UEP2:
                    ...
                    USBD_EP2_StartTransfer(); /* refill from the demo ring */
                    break;
...
/******************************************************************************/
/* WinUSB/WebUSB demo API shims - same surface as the CH592F FS stack, so
 * app code (main/button/filexfer) compiles unchanged on both peripherals. */

volatile uint8_t USBD_DevEnumStatus = 0;
...
void USBD_EP2_StartTransfer( void )
{
    ...
    /* Idempotent + atomic vs the USB ISR, which calls this on IN-complete:
     * only arm when EP2 is idle, and never let a completion interrupt split
     * the callback fill from the ACK arm. */
    PFIC_DisableIRQ( USB2_DEVICE_IRQn );
