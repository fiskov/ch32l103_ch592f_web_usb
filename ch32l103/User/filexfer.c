 ...
 *                       bulk IN for USB throughput testing.
 *
 ...
 *
 *  Producer/consumer ring buffer (uses some of the CH32L103's 20KB RAM):
 *  packet generation is done by FileXfer_Pump(), called repeatedly from
 *  the idle main loop, which fills a ring of pre-built 64-byte packets
 *  ahead of time. The USB interrupt handler's fill callback then just
 *  memcpy()s the next ready packet - minimal work in interrupt context,
 *  so the host-visible per-transaction turnaround time is dominated by
 *  the USB hardware/protocol itself rather than by CPU-side packet
 *  preparation. If the ring ever runs dry (e.g. right after
 *  FileXfer_Start(), before the main loop has had a chance to pump), the
 *  callback falls back to generating that one packet synchronously -
 *  correctness is preserved either way, only speed differs.
 *
 ...
 *
 *  The output byte VALUES exactly match webusb-demo/app.js's
 *  expectedFileByte() mirror, so every downloaded byte can be verified
 *  host-side with no reference file needed anywhere.
 *******************************************************************************/
#include "filexfer.h"
#include "ch32l103_usbfs_device.h"
#include <string.h>

#define ROW_SIZE     (FILEXFER_IMG_WIDTH * 3u)

/* Ring buffer of pre-built packets. 32 slots * 64 bytes = 2KB, a small
 * fraction of the 20KB total RAM. Must be a power of two for cheap
...
void FileXfer_Init(void)
{
    ...
    USBFS_EP2_SetFillCallback(FileXfer_FillCallback);
}
